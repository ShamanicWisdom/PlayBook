 ...
 *
 * Klasa kontrolerska finalizowania rezerwacji - ConfirmReservationController.java
 * 
 */
...
public class ConfirmReservationController 
{
    ...
    public void setDialogStage(Stage dialogStage, int accountID, int showID, int chosenSlotsCount, String chosenSlotsID)  throws PropertyVetoException, SQLException 
    {
    ...
    //1. Alert do braku lacznosci z baza.
        ...
        {  
            ...
            else
            {
                try
                {
                    parseNormal = Integer.parseInt(noNormalField.getText());
                }
            ...
            else
            {
                try
                {
                    parseHalf = Integer.parseInt(noHalfField.getText());
                }
                catch(NumberFormatException e)
                {
                    errorMessage += "Podano błędną liczbę miejsc!" + "\n";
            ...
            if(!errorMessage.contains("Poszę usunąć wszystkie znaki, które nie są cyframi!"))
            {
                int countedSlotsGivenFromFields = parseNormal + parseHalf;
    ...
    //Gdy liczba bledow jest rowna zero:
        if (errorMessage.length() == 0) 
        {
            ...
            Account account = connector.pullSingleAccount(accountID);
            
            connector.addReservation(account, show, chosenSlotsCount, chosenSlotsID, normalSlots, halfSlots, countedPrice);
            
            alert.setContentText("Dodawanie rezerwacji zakończone powodzeniem!");
            alert.showAndWait();
            return true;
        } 
    ...
    //Gdy liczba bledow jest rozna od zera:
        else 
        {
            ...
            alert.initOwner(dialogStage);
            alert.setTitle("Błąd!");
            alert.setHeaderText("Błąd dodawania rezerwacji!");
            alert.setContentText(errorMessage);
            alert.showAndWait();
            return false;
        }
