 ...
 *
 * Klasa kontrolerska edycji konta - EditAccountController.java.
 * 
 */
...
public class EditAccountController
{
    ...
    public void setDialogStage(Stage dialogStage, int accountID) 
    {
        List<Account> accountList = null;
        this.dialogStage = dialogStage;
        this.accountID = accountID;
        accountLevelList.addAll(Arrays.asList("admin", "operator", "user"));
        accountList = connector.pullAccountData(accountID);
        
        for(Account accountData : accountList)
        {
            nameField.setText(accountData.getName());
            nameField.setPromptText("Imię");
            surnameField.setText(accountData.getSurname());
            surnameField.setPromptText("Nazwisko");
            emailField.setText(accountData.getEmail());
            emailField.setPromptText("Adres email");
            passwordField.setText("");
            passwordField.setPromptText("Hasło (min. 6 znaków!)");
            phoneNumberField.setText(Integer.toString(accountData.getPhoneNumber()));
            phoneNumberField.setPromptText("Numer Telefonu");
            chosenLevel = accountData.getRole();
        }
        ...
        accountLevelBox.getSelectionModel().select(accountLevelList.indexOf(chosenLevel)); //wybranie aktualnego wojewodztwa.
        ...
        { 
            public void changed(ObservableValue observableValue, Number levelValue, Number newLevelValue) //observableValue - wybrana rzecz, value - stary indeks, newValue - nowy indeks (przy ewentualnej zmianie wyboru)
            {
    ...
    //1. Alert do braku lacznosci z baza.
        ...
        {              
            ...
            Matcher matchSurnameInitial = initialChar.matcher(nameField.getText()); //Matcher pobiera imie i porownuje , czy zgadza sie ze wzorcem.
            ...
            //2.6.1 Email nie jest ten sam, co poprzednio.
            if(!emailField.getText().equals(connector.pullUserEmail(accountID)))
            {
                //2.6.2 Email w uzyciu.
    ...
    //Gdy liczba bledow jest rowna zero:
        if (errorMessage.length() == 0) 
        {
            ...
            connector.editAccountData(nameField.getText(), surnameField.getText(), emailField.getText(), passwordField.getText(), phoneNumberField.getText(), chosenLevel, accountID);
            alert.setContentText("Edycja konta zakończone powodzeniem!");
            alert.showAndWait();
            return true;
        } 
    ...
    //Gdy liczba bledow jest rozna od zera:
        else 
        {
            ...
            alert.initOwner(dialogStage);
            alert.setTitle("Błąd!");
            alert.setHeaderText("Błąd edycji konta!");
