 ...
 *
 * Klasa kontrolerska dodawania teatru - AddTheatreController.java.
 * 
 */
...
public class AddTheatreController 
{
    ...
    private void handleOk() throws SQLException, ClassNotFoundException, PropertyVetoException 
    {
        if (validator()) 
        {
            theatre.setName(nameField.getText());
            theatre.setStreet(streetField.getText());
            theatre.setCity(cityField.getText());
            theatre.setVoivodeship(chosenVoivodeship);
            theatre.setPhoneNumber(Integer.parseInt(phoneNumberField.getText()));
            okClicked = true;
            dialogStage.close();
        }
    }
    ...
    //1. Alert do braku lacznosci z baza.
        ...
        //2. Alerty do zle wypelnionego pola nameField:
                ...
                else
                {
                    ...
                    {
                        errorMessage += "Ten teatr już istnieje!" + "\n";
                    } 
                }
            }
        }
    ...
    //Gdy liczba bledow jest rowna zero:
        if (errorMessage.length() == 0) 
        {
            ...
            connector.addTheatre(nameField.getText(), cityField.getText(), streetField.getText(), chosenVoivodeship, phoneNumberField.getText());
            alert.setContentText("Dodawanie teatru zakończone powodzeniem!");
            alert.showAndWait();
            return true;
        } 
    ...
    //Gdy liczba bledow jest rozna od zera:
        else 
        {
            ...
            alert.initOwner(dialogStage);
            alert.setTitle("Błąd!");
            alert.setHeaderText("Błąd dodawania teatru!");
