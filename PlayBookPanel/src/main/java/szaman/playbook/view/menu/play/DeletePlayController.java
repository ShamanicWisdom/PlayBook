 ...
 *
 * Klasa kontrolerska usuwania sztuki - DeletePlayController.java.
 * 
 */
...
public class DeletePlayController
{
    @FXML
    ...
    public void setDialogStage(Stage dialogStage, int playID, String adminEmail) throws SQLException, ClassNotFoundException 
    {
    ...
    {
        System.out.println("DeletePlay detected!"); //kontrola, czy dziala interakcja.
        if (validator()) 
        {
            okClicked = true;
            dialogStage.close();
    ...
    //Gdy liczba bledow jest rowna zero:
        if (errorMessage.length() == 0) 
        {
            ...
            connector.deletePlay(playID);
            alert.setContentText("Sztuka została usunięta!");
            alert.showAndWait();
            return true;
        } 
